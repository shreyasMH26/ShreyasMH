...
export default function Contact() {
  ...
  const handleCopyEmail = async () => {
    try {
      await navigator.clipboard.writeText('shreyasmholeyache@gmail.com');
      setCopied(true);
  ...
  return (
    <section
      ref={ref}
      id="contact"
      className={`relative z-20 bg-[#080808] text-white py-32 px-5 sm:px-8 md:px-12 flex flex-col justify-between min-h-[80vh] section-reveal ${isInView ? 'visible' : ''
        }`}
    >
            ...
            >
              <span className="flex items-center gap-3">
                ...
                <span className="text-[13px] sm:text-[15px]">
                  {copied ? 'Email Copied!' : 'shreyasmholeyache@gmail.com'}
                </span>
              </span>
